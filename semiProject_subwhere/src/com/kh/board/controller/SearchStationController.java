 ...
 */
@WebServlet("/searchStation.do")
public class SearchStationController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int stationNo = Integer.parseInt(request.getParameter("station-name"));
		...
		//*listCount : 총 게시글 개수
		listCount = new StationService().headerSearchListCount(stationNo);
		...
		ArrayList<Sight> list = new StationService().searchStation(pi, stationNo);
		Station headerSearch = new StationService().whatisStationName(stationNo);
		
		String headerStationName = headerSearch.getStationName();
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.setAttribute("headerSearch", headerStationName);
